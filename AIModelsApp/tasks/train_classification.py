...
def train_classification_model(ai_model, df, target_column):
    from pycaret.classification import setup, compare_models, tune_model, save_model
    
    try:
        logger.info(f"Iniciando clasificación. DF shape: {df.shape}")
        logger.info(f"Target column: {target_column}")
        logger.info(f"Columns disponibles: {list(df.columns)}")
        logger.info(f"Target values: {df[target_column].value_counts()}")
        ...
        # Compare models con modelos específicos
        ...
        logger.error(f"Error en train_classification_model: {str(e)}")
